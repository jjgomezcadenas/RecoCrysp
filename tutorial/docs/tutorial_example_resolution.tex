...
The cleanliness is set by the number of sampled LORs as much as by the counts.
A continuous scanner's randomly sampled LORs cover line space far less uniformly
than a structured sinogram, so resolving the fine rods cleanly needs dense
angular coverage --- $2\times10^7$ LORs here. Fewer LORs leave the
high-frequency rod structure under-sampled, and the reconstruction looks grainy
\emph{however many counts} are collected; the cure is more LORs, not more
counts.
